...
class SHACPlayer {
    ...
    setupFileInput() {
        // Skip file input setup in Electron (handled by electron-integration.js)
        if (window.electronAPI) {
            console.log('Skipping file input setup - using Electron dialog');
            // Still setup drag-and-drop
            this.setupDragDrop();
            return;
        }
    ...
    setupDragDrop() {
        ...
        // Browse button in overlay (only for web, Electron handles this separately)
        if (!window.electronAPI) {
